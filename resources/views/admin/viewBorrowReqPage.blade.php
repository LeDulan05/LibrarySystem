...
<head>
    ...
    <title>IskoLib - Borrow Request Details</title>
</head>
...
<body>
    <div class="layout-container">
        ...
        <main class="main-canvas">
            <div class="dashboard-header">
                <h1 class="dashboard-title">Borrow Request Details</h1>
    ...
    <div class="modal-overlay" id="approveModalOverlay">
        <form action="{{ route('admin.borrow.approve', $requestData->id) }}" method="POST" class="suspend-modal-card">
            @csrf
            <div class="modal-round-icon-frame success-frame"><i class="bi bi-check-circle"></i></div>
            <h3 class="modal-main-title">Approve Request?</h3>
            <p class="modal-warning-body">Confirming this action will mark this book as borrowed and generate a physical library checkout receipt tracking voucher log.</p>
            ...
            <div style="width: 100%; background: #F9F6F0; border: 1px solid #EAE6DF; border-radius: 12px; padding: 16px; margin-bottom: 24px;">
                <div class="info-grid-row" style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                    <span class="label" style="font-size: 0.85rem; font-weight: 600; color: #71717A;">Borrower</span>
    ...
    <div class="modal-overlay" id="rejectReasonModalOverlay">
        <form action="{{ route('admin.borrow.reject', $requestData->id) }}" method="POST" class="decision-modal-card" style="max-width: 480px;">
            @csrf
            <div class="modal-round-icon-frame danger-frame">
                <i class="bi bi-x-lg"></i>
            </div>
            
            <h3 class="modal-main-title">Reject Borrow Request?</h3>
            ...
            <div class="rejection-options-container">
                ...
                <label class="radio-option-row">
                    <input type="radio" name="reason" value="Out of Stock" required>
                    <span class="radio-text">Out of Stock</span>
                </label>
                
                <label class="radio-option-row">
                    <input type="radio" name="reason" value="Maximum Borrow Limit Reached">
                    <span class="radio-text">Maximum Borrow Limit Reached</span>
                </label>
                
                <label class="radio-option-row">
                    <input type="radio" name="reason" value="Invalid Account / Suspended Member">
                    <span class="radio-text">Invalid Account / Suspended Member</span>
                </label>
                
                <label class="radio-option-row">
                    <input type="radio" name="reason" value="Book Reserved by Another Member">
                    <span class="radio-text">Book Reserved by Another Member</span>
                </label>
            </div>

            <div class="suggestion-notice-box">
                <i class="bi bi-lightbulb" style="margin-right: 6px;"></i> Suggestion: You may recommend the member to place a reservation instead.
            </div>
